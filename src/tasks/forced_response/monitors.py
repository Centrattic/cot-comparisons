...
def _load_question_from_verification(
    ...
    rollout_idx: int,
    verbose: bool = True,
) -> Optional[Tuple[GPQAQuestion, str]]:
...
def parse_distribution(response: str) -> Tuple[bool, Dict[str, float]]:
    """Parse a distribution JSON from the response.

    Expected format: {"A": 0.1, "B": 0.7, "C": 0.1, "D": 0.1}
    ...
    for attempt in range(max_retries):
        async with semaphore:
            try:
                ...
                is_valid, distribution = parse_distribution(raw_response)
...
async def _run_single_resampling_monitor_job(
    ...
    """A single resampling monitor job: retries until a valid single-token answer."""
